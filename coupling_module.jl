...
function coup_model!(du, u, p, t)
        ...
        du[4] = (e_CR * a_CR * R2 * C2)/(1 + a_CR * h_PC * R2) - (a_PC * Ω * C2 * P)/(1 + a_PC * (1-Ω) * h_PC * C1 + a_PC * Ω * h_PC * C2) - m_C * C2
...
for i=1:length(coup_vals)
    ...
    sol = solve(prob, reltol = 1e-8, atol = 1e-8)
    grid = sol(ts)
    equ = nlsolve((du, u) -> coup_model!(du, u, p, 0.0), grid.u[end]).zero
